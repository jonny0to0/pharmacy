...
export const authenticateToken = async (req, res, next) => {
    ...
    try {
        ...
        if (isPrivileged && user.lastGeoLat && user.lastGeoLon && user.lastGeoTimestamp) {
            const { isImpossible, speed, risk } = GeoService.checkImpossibleTravel({ lat: user.lastGeoLat, lon: user.lastGeoLon, timestamp: user.lastGeoTimestamp }, { lat: currentGeo.lat, lon: currentGeo.lon, timestamp: now });
            if (risk === 'HIGH') {
        ...
        if (isPrivileged && !needsStepUp && user.lastKnownIp && user.lastKnownIp !== currentIp) {
            needsStepUp = true;
            await AlertingService.notify(`⚠️ [SESSIONS] New IP detected for Admin ${user.email}: ${currentIp}`, "WARNING", `NEW_IP_${user.id}`);
        }
 ...
 */
export const authorizePermission = (permission, requiredScope = 'LIMITED') => {
