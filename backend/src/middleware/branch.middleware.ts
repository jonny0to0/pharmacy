...
declare global {
  namespace Express {
    interface Request {
      allowedBranchIds?: string[]; // undefined/null means no restrictions (all branches)
    }
  }
}

export const checkBranchRestriction = async (req: Request, res: Response, next: NextFunction): Promise<any> => {
  if (!req.user) {
  ...
  try {
    ...
    const reqBranchId = (req.body?.branchId || req.query?.branchId) as string | undefined;
