...
export const checkFeature = (featureKey) => {
    return async (req, res, next) => {
        try {
            ...
            const isSuperAdmin = req.user?.roles?.some((r) => (typeof r === 'string' ? r : r.role?.name) === "SUPER_ADMIN");
            if (isSuperAdmin)
                return next();
            if (!snapshot) {
 ...
 */
export const checkLimit = async (tenantId, limitKey, currentCount) => {
    const snapshot = await PlanService.getTenantPlanSnapshot(tenantId);
    if (!snapshot)
    ...
    if (currentCount >= limit) {
        const error = new Error(`Limit reached for ${limitKey}. Current: ${currentCount}, Max: ${limit}`);
