...
export const maskGSTIN = (gstin?: string | null) => {
  if (!gstin) return null;
  if (gstin.length < 5) return gstin;
...
export const maskObject = (obj: any) => {
  if (!obj) return obj;
  const masked = { ...obj };
  ...
  if (masked.gstin) masked.gstin = maskGSTIN(masked.gstin);
