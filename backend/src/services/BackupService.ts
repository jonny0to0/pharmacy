...
export class BackupService {
  ...
  static async getLatestStatus() {
    ...
    const lastResult = (global as any)._lastRestoreResult || { duration: "---", verdict: "NOT_TESTED" };
  ...
  static async runRestoreSimulation() {
    ...
    try {
        ...
        (global as any)._lastRestoreResult = result;
        await PlatformStateService.recordDrillResult(result as any);

        await AlertingService.notify(`DR Simulation Complete. Verdict: ${verdict}. RTO: ${durationSec}s. Geo-Sync: ${geoSuccess ? 'PASSED' : 'FAILED'}`, 
          verdict === 'SUCCESS' ? 'INFO' : 'CRITICAL', 'DR_SIM');

        return result;
    } catch (err) {
        console.error("❌ [BackupService] DR Simulation failed:", err);
        const failedResult = { verdict: "CRASHED", duration: "---" };
        await PlatformStateService.recordDrillResult(failedResult as any);
        return failedResult;
    }
  }
