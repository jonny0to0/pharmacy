...
export class GeoService {
   ...
   */
  static getDistance(loc1: { lat: number, lon: number }, loc2: { lat: number, lon: number }): number {
   ...
   */
  static checkImpossibleTravel(prev: Location, current: Location): { isImpossible: boolean, speed: number, risk: 'LOW' | 'MEDIUM' | 'HIGH' } {
    ...
    if (timeHours <= 0) return { isImpossible: false, speed: 0, risk: 'LOW' };

    const speedKmh = distanceKm / timeHours;
    
    let risk: 'LOW' | 'MEDIUM' | 'HIGH' = 'LOW';
    if (speedKmh > SECURITY_THRESHOLDS.GEO_SPEED_KMH) risk = 'HIGH';
    else if (distanceKm > 500) risk = 'MEDIUM'; // New country or far region
   ...
   */
  static mockLookup(locationHint: string): { lat: number, lon: number } {
