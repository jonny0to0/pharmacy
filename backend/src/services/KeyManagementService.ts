...
export class KeyManagementService {
  private static manifest: KeyVersion[] = [];
   ...
   */
  static rotateKey(reason: string) {
    ...
    this.manifest = this.manifest.map(kv => 
      kv.status === 'active' ? { ...kv, status: 'archived' } : kv
    );
   ...
   */
  static revokeKey(id: string, reason: string) {
  ...
  static getActiveKey() {
    ...
    if (!active) throw new Error("No active key found in KMS");
  ...
  static getPublicKey(id: string) {
    this.ensureInitialized();
    const key = this.manifest.find(k => k.id === id);
    if (!key) throw new Error(`Key version ${id} not found in manifest`);
  ...
  static getKeyImpactReport(id: string) {
    ...
    const dependentAnchors = anchors.filter((a: any) => a.keyId === id);
  ...
  private static ensureInitialized() {
    if (this.manifest.length === 0) this.initialize();
  }
}
