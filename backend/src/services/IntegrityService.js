...
export class IntegrityService {
    ...
    static async createDailyAnchor() {
        ...
        if (!latestLog)
            return;
        const lastHash = latestLog.metadata?.currentHash;
     ...
     */
    static async revokeKey(keyId, reason) {
    ...
    static async verifyAgainstAnchor(finalHash) {
        if (!fs.existsSync(ANCHOR_FILE))
            return { success: true, message: "No anchors to verify" };
        const anchors = JSON.parse(fs.readFileSync(ANCHOR_FILE, 'utf8'));
        if (anchors.length === 0)
            return { success: true, message: "Anchor list empty" };
        ...
        const isSignedCorrectly = verify("sha256", Buffer.from(latestAnchor.hash), {
            key: publicKey,
            padding: constants.RSA_PKCS1_PSS_PADDING,
    ...
    static async reportViolation(logId, reason) {
    ...
    static extractOriginalDetails(metadata) {
