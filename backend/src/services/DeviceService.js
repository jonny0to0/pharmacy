...
export class DeviceService {
    static SECRET = process.env.JWT_SECRET || 'trusted-device-secret-key-123';
    /**
     * Generates a signed token for a trusted device
     */
    static generateToken(userId, deviceId) {
        const payload = {
            userId,
            deviceId,
     ...
     */
    static async verifyDevice(userId, token) {
        try {
