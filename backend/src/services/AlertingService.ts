...
class ConsoleProvider implements AlertProvider {
  async send(message: string, severity: string) {
...
export class AlertingService {
  private static providers: AlertProvider[] = [];
   ...
   */
  static async notify(message: string, severity: 'INFO' | 'WARNING' | 'CRITICAL' = 'INFO', cooldownKey?: string) {
    this.initialize();
    ...
    if (upperMsg.includes('INTEGRITY') || upperMsg.includes('BREACH')) runbookUrl = RUNBOOKS.INTEGRITY;
    else if (upperMsg.includes('DB') || upperMsg.includes('DATABASE')) runbookUrl = RUNBOOKS.DATABASE;
    else if (upperMsg.includes('PAYMENT')) runbookUrl = RUNBOOKS.PAYMENTS;
    else if (upperMsg.includes('SECURITY') || upperMsg.includes('AUTH')) runbookUrl = RUNBOOKS.SECURITY;
  ...
  static async getIntelligenceReport() {
    ...
    const typeAnalysis: Record<string, { total: number, sumMttr: number }> = {};
    resolved.forEach(r => {
       const type = r.runbookUrl?.split('/').pop()?.replace('.md', '') || 'unknown';
       const mttr = (r.resolvedAt!.getTime() - r.createdAt.getTime()) / 60000;
       if (!typeAnalysis[type]) typeAnalysis[type] = { total: 0, sumMttr: 0 };
  ...
  static async runEscalationCheck() {
    ...
    for (const incident of staleCriticals) {
      await this.notify(
        `[SLA BREACH] Critical Incident ${incident.id} unacknowledged for > 5m!`,
        'CRITICAL',
        `SLA_BREACH_${incident.id}`
      );
    }
