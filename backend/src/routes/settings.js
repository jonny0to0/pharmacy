...
router.get("/full-profile", authenticateToken, async (req, res) => {
    try {
        ...
        const responseData = JSON.parse(JSON.stringify(tenant));
...
router.post("/profile", authenticateToken, requirePermission("SETTINGS_BUSINESS.UPDATE"), async (req, res) => {
    try {
        const tenantId = req.user.tenantId;
        if (!tenantId)
            return res.status(400).json({ error: "Tenant ID required" });
        const { businessName, ownerName, phone, email, address, state, pinCode, logoKey, bannerKey, pan, drugLicense, fssai, gstUrl, drugLicenseUrl, panUrl, fssaiUrl } = req.body;
...
router.post("/tax-billing", authenticateToken, requirePermission("SETTINGS_BUSINESS.UPDATE"), async (req, res) => {
    try {
        const tenantId = req.user.tenantId;
...
router.post("/modules", authenticateToken, requirePermission("SETTINGS_BUSINESS.UPDATE"), async (req, res) => {
    try {
        const tenantId = req.user.tenantId;
        const { enableMedicalInfo, walkInCustomerBehavior, allowPharmacistCustomerCreation, restrictedMenuBehavior } = req.body;
        await prisma.tenantsettings.upsert({
            where: { tenantId },
            update: { enableMedicalInfo, walkInCustomerBehavior, allowPharmacistCustomerCreation, restrictedMenuBehavior },
            create: { id: randomUUID(), tenantId, enableMedicalInfo, walkInCustomerBehavior, allowPharmacistCustomerCreation, restrictedMenuBehavior }
        });
...
router.post("/personal-profile", authenticateToken, async (req, res) => {
    try {
