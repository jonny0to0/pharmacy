...
router.get("/dashboard", authenticateToken, enforceTenantScope, async (req, res) => {
    try {
        ...
        if (isSuperAdmin) {
            ...
            const topBusinessesEnriched = await Promise.all(topBusinesses.map(async (b) => {
...
router.get("/sales-summary", authenticateToken, enforceTenantScope, async (req, res) => {
    try {
...
router.get("/daily-closing", authenticateToken, enforceTenantScope, async (req, res) => {
    try {
        const tenantId = req.user.tenantId;
...
router.get("/purchase-summary", authenticateToken, enforceTenantScope, async (req, res) => {
    try {
        const { startDate, endDate } = req.query;
        const tenantId = req.user.tenantId;
        const where = { tenantId: tenantId };
...
router.get("/inventory-status", authenticateToken, enforceTenantScope, async (req, res) => {
    try {
        const tenantId = req.user.tenantId;
...
router.get("/profit-loss", authenticateToken, enforceTenantScope, async (req, res) => {
    try {
        const { startDate, endDate } = req.query;
        const tenantId = req.user.tenantId;
        const where = { tenantId: tenantId };
...
router.get("/gst-summary", authenticateToken, enforceTenantScope, async (req, res) => {
    try {
        const { startDate, endDate } = req.query;
        const tenantId = req.user.tenantId;
        const where = { tenantId: tenantId };
...
// Export Report
router.get("/export", authenticateToken, enforceTenantScope, async (req, res) => {
    try {
        const tenantId = req.user.tenantId;
