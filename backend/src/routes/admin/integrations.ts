 ...
 */
router.get("/", authorizePermission("manage_integrations"), async (req: Request, res: Response) => {
  try {
 ...
 */
router.post("/", authorizePermission("manage_integrations", "FULL"), async (req: Request, res: Response) => {
  try {
    ...
    if (error.code === 'P2002') return res.status(400).json({ success: false, error: "Integration name must be unique" });
 ...
 */
router.patch("/:id", authorizePermission("manage_integrations", "FULL"), async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
