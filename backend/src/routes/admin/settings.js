 ...
 */
router.get("/", authorizePermission("manage_platform_settings"), async (req, res) => {
    try {
 ...
 */
router.put("/", authorizePermission("manage_platform_settings", "FULL"), async (req, res) => {
    try {
        ...
        await prisma.$transaction(settings.map(s => prisma.systemSetting.upsert({
            where: { key: s.key },
