 ...
 */
router.post("/tenant/:tenantId/reset", sensitiveActionsLimiter, async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    if (!tenant) return res.status(404).json({ error: "Target business not found." });
 ...
 * Risk Level: HIGH
 */
router.post("/tenant/:tenantId/suspend", sensitiveActionsLimiter, async (req: Request, res: Response): Promise<any> => {
  try {
    const { tenantId } = req.params;
