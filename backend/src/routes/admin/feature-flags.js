 ...
 */
router.get("/", authorizePermission("manage_flags"), async (req, res) => {
    try {
 ...
 */
router.post("/", authorizePermission("manage_flags", "FULL"), async (req, res) => {
    try {
        ...
        if (error.code === 'P2002')
            return res.status(400).json({ success: false, error: "Flag key must be unique" });
 ...
 */
router.patch("/:id", authorizePermission("manage_flags", "FULL"), async (req, res) => {
    try {
        const { id } = req.params;
 ...
 */
router.delete("/:id", authorizePermission("manage_flags", "FULL"), async (req, res) => {
    try {
        const { id } = req.params;
