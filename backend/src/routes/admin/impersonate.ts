 ...
 */
router.post("/:tenantId", sensitiveActionsLimiter, async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    const impersonationToken = jwt.sign(
      ...
      secret,
      { expiresIn: '30m' } // Short duration for safety
    );
    ...
    await createAuditLog(
      ...
      "IMPERSONATION_START", 
      tenantId, 
      { reason, targetBusiness: tenant.businessName },
      "CRITICAL",
      req.ip
    );
 ...
 */
router.post("/stop", sensitiveActionsLimiter, async (req: Request, res: Response): Promise<any> => {
  try {
    // Audit Log the exit
    await createAuditLog(
      req.user!.userId, 
      ...
      { message: "Administrative session terminated normally" },
      "WARNING", // Elevated log level per production decision
      req.ip
    );
