...
// Request Leave
router.post("/request", authenticateToken, auditLog("LEAVE_REQUEST", "LEAVES"), async (req: Request, res: Response) => {
  try {
    const tenantId = req.user!.tenantId;
    const userId = req.user!.userId;
...
router.patch("/:id/status", authenticateToken, auditLog("LEAVE_STATUS_UPDATE", "LEAVES"), async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
