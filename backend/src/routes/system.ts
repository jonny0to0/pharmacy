...
router.get("/health", authenticateToken, authorizeRoles("SUPER_ADMIN"), async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    return sendSuccess(res, {
      ...
      monitoring: {
         errorRate5m: (global as any)._errorStats?.rate || 0,
         p95Latency: "124ms",
         p99Latency: "410ms", 
         availability: "99.98%",
         since: (global as any)._lastPerformanceDip || "2026-04-19T12:00:00Z"
      },
      services: {
      ...
      incidents: activeIncidents.map(i => ({
          ...i,
          assignedTo: (i as any).user?.name || null,
 ...
 */
router.post("/integrity/rotate-keys", authenticateToken, authorizeRoles("SUPER_ADMIN"), async (req: Request, res: Response) => {
   try {
 ...
 */
router.post("/incidents/:id/assign", authenticateToken, authorizeRoles("SUPER_ADMIN"), async (req: Request, res: Response) => {
   try {
...
router.post("/incidents/:id/note", authenticateToken, authorizeRoles("SUPER_ADMIN"), async (req: Request, res: Response) => {
   try {
      const { note } = req.body;
...
router.post("/incidents/:id/resolve", authenticateToken, authorizeRoles("SUPER_ADMIN"), async (req: Request, res: Response) => {
   try {
 ...
 */
router.post("/backup/restore-simulation", authenticateToken, authorizeRoles("SUPER_ADMIN"), async (req: Request, res: Response) => {
   try {
 ...
 */
router.post('/integrity/verify', authenticateToken, async (req: Request, res: Response): Promise<any> => {
 ...
 */
router.get("/security/devices", authenticateToken, authorizeRoles("SUPER_ADMIN"), async (req: Request, res: Response) => {
    try {
...
router.delete("/security/devices/:id", authenticateToken, authorizeRoles("SUPER_ADMIN"), async (req: Request, res: Response) => {
    try {
 ...
 */
router.get("/integrity/key-impact/:id", authenticateToken, authorizeRoles("SUPER_ADMIN"), async (req: Request, res: Response) => {
    try {
