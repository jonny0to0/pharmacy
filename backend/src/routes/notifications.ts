 ...
 * @access  Private
 */
router.get("/preferences", authenticateToken, async (req: Request, res: Response): Promise<any> => {
  try {
 ...
 * @access  Private
 */
router.put("/:id/read", authenticateToken, async (req: Request, res: Response): Promise<any> => {
  try {
    const userId = req.user!.userId;
    const { id } = req.params;
