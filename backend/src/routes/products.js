...
// Get all products
router.get("/", authenticateToken, requirePermission("PRODUCTS.READ"), async (req, res) => {
    try {
...
router.post("/", authenticateToken, requirePermission("PRODUCTS.CREATE"), auditLog("CREATE_PRODUCT", "PRODUCTS"), validate(productSchema), async (req, res) => {
    try {
        const { name, sku, barcode, category, hsnCode, unit, manufacturer, purchasePrice, sellingPrice, mrp, currentStock, gstRate, minStockLevel, location, imageKey, medicalDescription, uses, contraindications, sideEffects, precautions, dosageInfo } = req.body;
        // Validation logging
...
router.post("/create-with-batch", authenticateToken, requirePermission("PRODUCTS.CREATE"), auditLog("CREATE_PRODUCT_WITH_BATCH", "PRODUCTS"), validate(productWithBatchSchema), async (req, res) => {
    try {
        ...
        const result = await prisma.$transaction(async (tx) => {
            ...
            const newProduct = await tx.product.create({
                data: {
                    ...
                    hasMedicalInfo: !!(product.medicalDescription ||
                        product.uses ||
...
// Update product
router.put("/:id", authenticateToken, requirePermission("PRODUCTS.UPDATE"), auditLog("UPDATE_PRODUCT", "PRODUCTS"), validate(productSchema), async (req, res) => {
    try {
        const { id } = req.params;
        const tenantId = req.user.tenantId;
        const { name, sku, purchasePrice, category, hsnCode, unit, sellingPrice, mrp, gstRate, minStockLevel, currentStock, location, imageKey, medicalDescription, uses, contraindications, sideEffects, precautions, dosageInfo } = req.body;
        const hasMedicalInfo = !!(medicalDescription ||
            uses ||
            contraindications ||
            sideEffects ||
            precautions ||
...
// Delete product
router.delete("/:id", authenticateToken, requirePermission("PRODUCTS.DELETE"), auditLog("DELETE_PRODUCT", "PRODUCTS"), async (req, res) => {
    try {
        const { id } = req.params;
        const tenantId = req.user.tenantId;
        ...
        const hasRelations = product.saleinvoiceitem.length > 0 ||
            product.purchasebillitem.length > 0 ||
            product.purchaseorderitem.length > 0 ||
            product.salesorderitem.length > 0 ||
            product.stockadjustment.length > 0;
        if (hasRelations) {
            // Soft delete
...
router.get("/low-stock", authenticateToken, requirePermission("PRODUCTS.READ"), async (req, res) => {
    try {
