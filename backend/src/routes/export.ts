 ...
 */
router.get("/sales", authenticateToken, authorizeRoles("BUSINESS_ADMIN", "MANAGER"), async (req: Request, res: Response) => {
  try {
 ...
 * @access  Private (Admin only)
 */
router.get("/inventory", authenticateToken, authorizeRoles("BUSINESS_ADMIN", "MANAGER"), async (req: Request, res: Response) => {
  try {
    const tenantId = req.user!.tenantId;
