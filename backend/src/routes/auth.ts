...
router.post("/register", validate(registerSchema), async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    await basePrisma.session.upsert({
      where: { userId_userAgent_ip: { userId: result.user.id, userAgent: req.headers['user-agent'] as string || '', ip: req.ip || '' } },
...
router.post("/login", validate(loginSchema), async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    await basePrisma.session.upsert({
      where: { userId_userAgent_ip: { userId: user.id, userAgent: req.headers['user-agent'] as string || '', ip: req.ip || '' } },
      update: { token: refreshToken, expiresAt: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000) },
      create: { 
        id: randomUUID(),
        userId: user.id, 
