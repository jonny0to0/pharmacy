...
router.get("/current", authenticateToken, async (req, res) => {
    try {
        ...
        delete tenantWithSubscriptions.subscription;
        // 🔒 PII Masking during impersonation
        let data = tenantWithSubscriptions;
        if (req.user?.isImpersonating) {
            data = {
                ...tenantWithSubscriptions,
                subscriptions: tenantWithSubscriptions.subscriptions.map((s) => maskObject(s))
            };
        }
        res.json(data);
    }
...
router.post("/change", authenticateToken, async (req, res) => {
    try {
        ...
        const subscription = await SubscriptionService.createSubscription(tenantId, planCode, billingCycle, {
            amount,
