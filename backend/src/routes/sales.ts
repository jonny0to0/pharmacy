...
router.get("/", authenticateToken, requirePermission("SALES.READ"), async (req: Request, res: Response) => {
  try {
    ...
    const mappedInvoices = invoices.map(inv => ({
      ...inv,
      items: (inv as any).saleinvoiceitem
    }));
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      for (const item of items) {
        ...
        if (item.batchId) {
          ...
          if (!batch || batch.productId !== product.id) throw new Error(`Batch not found for product ${product.name}`);
      ...
      if (customerId) {
        ...
        if (!customer || customer.tenantId !== tenantId) throw new Error("Customer not found or access denied.");
      }
