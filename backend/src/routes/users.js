...
router.get("/staff", authenticateToken, requirePermission("STAFF.READ"), async (req, res) => {
    try {
...
// Add New Staff
router.post("/staff", authenticateToken, requirePermission("STAFF.CREATE"), async (req, res) => {
    try {
        const tenantId = req.user.tenantId;
        if (!tenantId)
            return res.status(400).json({ error: "Tenant ID required" });
        const { name, email, mobile, role, employeeId, department, designation, employmentType, joinDate, salary, workShift, reportingManagerId } = req.body;
...
// Update Staff
router.put("/staff/:id", authenticateToken, requirePermission("STAFF.UPDATE"), async (req, res) => {
    try {
        const tenantId = req.user.tenantId;
        const id = req.params.id;
        const { name, email, mobile, role, isActive, employeeId, department, designation, employmentType, joinDate, salary, workShift, reportingManagerId } = req.body;
...
// Delete Staff
router.delete("/staff/:id", authenticateToken, requirePermission("STAFF.DELETE"), async (req, res) => {
    try {
        const tenantId = req.user.tenantId;
        const id = req.params.id;
        const staff = await prisma.user.findFirst({
...
// Resend Invitation
router.post("/staff/:id/resend-invite", authenticateToken, requirePermission("STAFF.CREATE"), async (req, res) => {
    try {
        const tenantId = req.user.tenantId;
        const id = req.params.id;
        const staff = await prisma.user.findFirst({
...
router.get("/staff/:id/permissions", authenticateToken, requirePermission("ROLES.READ"), async (req, res) => {
    try {
        const tenantId = req.user.tenantId;
        const { id } = req.params;
...
router.put("/staff/:id/permissions", authenticateToken, requirePermission("ROLES.UPDATE"), async (req, res) => {
    try {
        const tenantId = req.user.tenantId;
        const { id } = req.params;
