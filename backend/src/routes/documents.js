...
// Upload Document
router.post("/upload", authenticateToken, uploadMiddleware.single("file"), auditLog("DOCUMENT_UPLOAD", "STAFF_DOCS"), async (req, res) => {
    try {
...
router.get("/:id", authenticateToken, auditLog("DOCUMENT_VIEW", "STAFF_DOCS"), async (req, res) => {
    try {
        const { id } = req.params;
        const tenantId = req.user.tenantId;
        const userId = req.user.userId;
