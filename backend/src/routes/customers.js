...
router.get("/", authenticateToken, requirePermission("CUSTOMERS.READ"), async (req, res) => {
    try {
        const customers = await prisma.customer.findMany({
            where: { isDeleted: false, tenantId: req.user.tenantId },
...
// Create customer
router.post("/", authenticateToken, requirePermission("CUSTOMERS.CREATE"), validate(customerSchema), async (req, res) => {
    try {
        ...
        if (phone) {
            const existing = await prisma.customer.findUnique({
                where: { phone_tenantId: { phone, tenantId: tenantId } }
            });
            if (existing) {
...
// Update customer
router.put("/:id", authenticateToken, requirePermission("CUSTOMERS.UPDATE"), validate(customerSchema), async (req, res) => {
    try {
        const { id } = req.params;
        const tenantId = req.user.tenantId;
        const { name, phone, email, gst_number, address, state, customerType, creditLimit, creditDays, dob, gender, membershipType } = req.body;
        ...
        if (phone) {
            const existing = await prisma.customer.findFirst({
                where: { phone: phone, tenantId: tenantId, id: { not: id } }
            });
            if (existing) {
        ...
        if (creditDays !== undefined)
            updateData.creditDays = Number(creditDays);
        if (dob !== undefined)
