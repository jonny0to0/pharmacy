...
router.post("/stripe", express.raw({ type: 'application/json' }), async (req, res) => {
    ...
    const loggedEvent = await WebhookService.recordEvent(subscriptionpayment_provider.STRIPE, event.id, event.type, event);
    if (!loggedEvent)
        return res.send({ received: true }); // Already processed
    try {
        ...
        await WebhookService.markFailed(loggedEvent.id, err);
...
router.post("/razorpay", express.raw({ type: 'application/json' }), async (req, res) => {
    ...
    // 1. Log Event & Check Idempotency
    const loggedEvent = await WebhookService.recordEvent(subscriptionpayment_provider.RAZORPAY, eventId, eventType, payload);
    if (!loggedEvent)
