...
async function backfill() {
  ...
  const getPlanId = (code: string) => plans.find(p => p.code === code)?.id;

  for (const tenant of tenants) {
    try {
      const oldStatus = tenant.subscriptions[0]?.status as string || 'INACTIVE';
      const oldPlan = tenant.currentPlan || 'FREE';

      // 🗺️ Map states according to enterprise logic
      let newState: SubscriptionStatus;
      switch (oldStatus) {
        case 'TRIALING': newState = SubscriptionStatus.TRIAL; break;
        case 'ACTIVE': newState = SubscriptionStatus.ACTIVE; break;
        case 'PAST_DUE': newState = SubscriptionStatus.PAST_DUE; break;
        case 'SUSPENDED': newState = SubscriptionStatus.SUSPENDED; break;
        case 'CANCELLED': newState = SubscriptionStatus.CANCELLED; break;
        case 'ARCHIVED': newState = SubscriptionStatus.EXPIRED; break;
        case 'EXPIRED': newState = SubscriptionStatus.EXPIRED; break;
        default: 
      ...
      if (!targetPlanId) throw new Error(`Missing Plan record for code: ${oldPlan}`);
